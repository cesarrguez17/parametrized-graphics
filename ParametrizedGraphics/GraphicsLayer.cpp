...
void CGraphicsLayer::InitD3D(int Width,int Height,int bpp)
{
	...
	bool result = m_Input->Initialize(GetModuleHandle(NULL), m_hWnd, Width, Height);
	if(!result)
	{
...
HRESULT CGraphicsLayer::CreateConstantsBuffer()
{
	...
	Desc.ByteWidth = sizeof( CONSTANTS_LIGHTS );
	hr=m_pDevice->CreateBuffer( &Desc, NULL, &m_pConstantLightsBuffer );
    return hr;
...
int CGraphicsLayer::CreateShader()
{
	...
	// Compile shaders
	 ...
	 ID3D10Blob* pBlobPST = NULL;
	 ID3D10Blob* pBlobShellPS = NULL;
	 
	 ID3D10Blob* pBlobGS = NULL;
	 ID3D10Blob* pBlobShellGS = NULL;
  ...
  //init constants
	...
	m_pDeviceContext->Map( m_pConstantLightsBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedResource );
    CONSTANTS_LIGHTS* pData = ( CONSTANTS_LIGHTS* )MappedResource.pData;

	pData->g_Ambient = D3DXVECTOR4(.3f, 0.3f, 0.3f, 1.0f);
	...
	m_pDeviceContext->Unmap( m_pConstantLightsBuffer, 0 );
...
void CGraphicsLayer::UpdateMatrices()
{
	...
	m_Input->GetMouseLocation(x, y);
	float xMouse = (x - m_rcScreenRect.right*.5) /m_rcScreenRect.right;
	float yMouse = (y - m_rcScreenRect.bottom*.5)/m_rcScreenRect.bottom;
	pData->g_vMouse = D3DXVECTOR4(xMouse,-yMouse, 3.5, 1);
...
void CGraphicsLayer::SetMatrices()
{
	...
	m_pDeviceContext->GSSetConstantBuffers(0, 1, &m_pConstantsBuffer);
	m_pDeviceContext->PSSetConstantBuffers(0, 1, &m_pConstantsBuffer);
	m_pDeviceContext->PSSetConstantBuffers(1, 1, &m_pConstantLightsBuffer);
}
